...
class SearchService:
    ...
    def get_query_embedding(query: str) -> list[float] | None:
        ...
        try:
            ...
            logger.error(f"Embedding error for query: {query[:50]}...", exc_info=True)
            return None
    ...
    def semantic_bill_search(embedding: list[float], threshold: float = 0.72, limit: int = 20) -> list[dict]:
        ...
        try:
            ...
            logger.exception("Database error during semantic search")
            return []
    ...
    @classmethod
    def _find_exact_bill_match(cls, query: str) -> Bill | None:
    ...
    @classmethod
    def _get_semantic_search_results(cls, query: str) -> tuple[list[int], dict[int, float]]:
        embedding = cls.get_query_embedding(query)
        if not embedding:
            return [], {}
        
        semantic_results = cls.semantic_bill_search(embedding)
        ids = [r["bill_idp"] for r in semantic_results]
        scores = {r["bill_idp"]: r["similarity"] for r in semantic_results}
        return ids, scores

    @classmethod
    def _extract_entities_from_query(cls, query: str) -> dict:
        ...
        return {
            ...
            "names": matched_names,
            "law_tokens": law_tokens
        }

    @classmethod
    def _build_bill_search_queryset(cls, semantic_ids: list[int], semantic_scores: dict, entity_matches: dict, exclude_idp: int | None = None):
        ...
        law_tokens = entity_matches["law_tokens"]
        matched_names = entity_matches["names"]
        effective_law_tokens = law_tokens if law_tokens else matched_names
        
        hybrid_filter = Q()
        if semantic_ids:
            hybrid_filter |= Q(idp__in=semantic_ids)
        ...
        if exclude_idp:
            bill_queryset = bill_queryset.exclude(idp=exclude_idp)
        ...
        return list(bill_queryset.distinct()[:200])

    @classmethod
    def _build_mp_search_queryset(cls, matched_law_codes: list[str], entity_matches: dict):
        matched_names = entity_matches["names"]
        matched_counties = entity_matches["counties"]
        matched_parties = entity_matches["parties"]
        ...
        return list(mp_queryset.filter(mp_filter).order_by('mp_name').distinct())

    @classmethod
    def _build_relation_map(cls, query: str, matched_law_codes: list[str], mps: list[Parliamentarian]):
        if not matched_law_codes or not mps:
            return {}

        v_rows = MPVote.objects.filter(
            vote_session__bill__bill_number__in=matched_law_codes,
            parliamentarian__in=mps
        ).values(
    ...
    def execute_global_search(cls, raw_query: str):
        ...
        # 1. Decompose query and find exact match
        exact_bill = cls._find_exact_bill_match(query)
        semantic_ids, semantic_scores = cls._get_semantic_search_results(query)
        entity_matches = cls._extract_entities_from_query(query)

        # 2. Fetch bills
        bill_list = cls._build_bill_search_queryset(
            semantic_ids, 
            semantic_scores, 
            entity_matches, 
            exclude_idp=exact_bill.idp if exact_bill else None
        )

        # 3. Fetch MPs based on matched bills and entities
        matched_law_codes = [b.bill_number for b in bill_list if b.bill_number]
        if exact_bill and exact_bill.bill_number:
            matched_law_codes.append(exact_bill.bill_number)
        matched_law_codes = list(set(matched_law_codes))
        
        mp_list = cls._build_mp_search_queryset(matched_law_codes, entity_matches)

        # 4. Map relations
        relation_map = cls._build_relation_map(query, matched_law_codes, mp_list)

        logger.info(f"Search executed in {time.time() - start_time:.4f}s for query: '{query[:50]}'")
        return {
            'query': query,
