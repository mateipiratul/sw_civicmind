...
from .services import SearchService, FeedService
...
class BillViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def trending(self, request):
        ...
        for bill in recent_bills:
            ...
            if analysis and analysis.rel_impact_categories.exists():
                for category in analysis.rel_impact_categories.values_list('name', flat=True):
    ...
    def votes(self, request, pk=None):
        ...
        # Use VoteAnalyticsService to get vote buckets
    ...
    def personalized(self, request):
        ...
        # Use FeedService to get personalized bills
        queryset = self.filter_queryset(self.get_queryset())
        ...
        if page is not None:
            ...
            # Use FeedService to get representative MPs
            vote_limit = 5
            reps_queryset = FeedService.get_representatives(
                county=county, 
                preferred_party=preferred_party,
                limit=6
            )
            reps_serializer = ParliamentarianVoteMapSerializer(
                reps_queryset, many=True,
            ...
            response.data.update({
                ...
                'myRepresentatives': {
                    'total': reps_queryset.count() if hasattr(reps_queryset, 'count') else len(reps_queryset),
        ...
        return Response({
            ...
            'filters': {
                'laws': {
                    ...
                    'categories': sorted({cat for b in bill_list if b.ai_analysis for cat in b.ai_analysis.rel_impact_categories.values_list('name', flat=True) if cat})[:20],
                },
                'mps': {
